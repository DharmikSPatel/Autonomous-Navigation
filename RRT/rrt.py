import matplotlib.pyplot as plt
import matplotlib.patches as patches
import matplotlib.animation as animation
import numpy as np
import argparse
import utils
import component_1, nearest_neighbors, collision_checking
from utils import Graph
import time
EXPORT = False
ITER_TESTING = 10
PART_6_TESTING = False
RUN, K, sample_goal, max_steer_dist =  500 if PART_6_TESTING else 1000, 1, .05, .7
def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--robot', choices=['arm', 'freeBody'])
    parser.add_argument('--start', nargs='*', type=float)
    parser.add_argument('--goal', nargs='*', type=float)
    parser.add_argument('--goal_rad', type=float)
    parser.add_argument('--map')

    args = parser.parse_args()
    return args

def make_rrt(obstacles, robot_type, start_pose, goal_pose, goal_rad, rand_pose, near, collisonOccurs, steer, dist):

    tree = Graph(directed=True, animation=True)
    start_pose = start_pose[:, np.newaxis]
    goal_pose = goal_pose[:, np.newaxis]
    tree.add_vertex(start_pose)
    amount_goal_samp = 0
    while len(tree._graph) <= RUN:
        if len(tree._graph) % 100 == 0:
            print("Node", len(tree._graph))
        pose_rand = rand_pose()
        if np.random.rand() <= sample_goal and not tree.in_graph(goal_pose):
            amount_goal_samp += 1
            pose_rand = goal_pose
        while collisonOccurs(pose_rand, obstacles)[0]:
            pose_rand = rand_pose()
        pose_nearest = (near(robot_type, K, pose_rand.flatten(), tree.get_vertices()))[0][:, np.newaxis]
        path, pose_new = steer(pose_nearest, pose_rand, dist, max_steer_dist)
        if all([not collisonOccurs(n, obstacles)[0] for n in path]):
            tree.add_edge(pose_nearest, pose_new)
            # comment in this code if you want RRT to stop as soon as goal region is found
            # if dist(pose_new.flatten(), goal_pose.flatten()) <= goal_rad:
            #     path, cost = utils.A_star(tree, dist, start, pose_new)
            #     print("sampled goal percentage:", amount_goal_samp/len(tree._graph))

            #     return path, cost, tree
    # print("sampled goal percentage:", amount_goal_samp/len(tree._graph))
    path, cost = utils.A_star(tree, dist, start_pose, goal_pose, goal_rad=goal_rad)
    return path, cost, tree 

    # dist(pose_new.flatten(), goal_pose.flatten()) <= goal_rad:

def main(args, animate=False):
    obstacles = component_1.scene_from_file(args.map)
    start = np.array(args.start)
    goal = np.array(args.goal)
    if args.robot == 'freeBody':
        start[2] = utils.normalize_angle(start[2])
        goal[2] = utils.normalize_angle(goal[2])
    else:
        start = utils.normalize_angle(start)
        goal = utils.normalize_angle(goal)
    
    if PART_6_TESTING:
        start_time = time.process_time()
    if args.robot == 'freeBody':
        path, cost, tree = make_rrt(obstacles, 
                        args.robot,
                        start,
                        goal,
                        args.goal_rad,
                        utils.random_pose_xytheta, 
                        nearest_neighbors.get_k_nearest, 
                        collision_checking.check_col_freeBody,
                        utils.steer_interpolate_freeBody,
                        nearest_neighbors.dist_freeBody)
    else:
        path, cost, tree = make_rrt(obstacles, 
                        args.robot,
                        start,
                        goal,
                        args.goal_rad,
                        utils.random_pose_thetas, 
                        nearest_neighbors.get_k_nearest, 
                        collision_checking.check_col_arm,
                        utils.steer_interpolate_arm,
                        nearest_neighbors.dist_arm)
    # delete
    # fig, ax = plt.subplots(1, 1, figsize=(15, 8))
    # ax.set_aspect('equal')
    # ax.set_xlim((-10,10))
    # ax.set_ylim((-10,10))
    # ax.grid()
    # component_1.visualize_scene_diff_axes(ax, obstacles)
    # utils.draw_graph(ax, tree, args.robot)
    # plt.show()
    # delete

    if PART_6_TESTING:
        end_time = time.process_time()
        
    path_found = True
    if path is None:
        print("No Path Found")
        path_found = False
        if not PART_6_TESTING or EXPORT:
            exit()

    if not animate:
        print(cost)
        dur = -1 if not PART_6_TESTING else end_time-start_time
        return path_found, dur, cost
    
    path = np.array(path).T  #DoF x len(path). each col vector is a pose
    print("Len Of Path", path.shape[1])
    
    
    final_path = []
    for i in range(path.shape[1]-1):
        pose1 = path[:, i][:, np.newaxis]
        pose2 = path[:, i+1][:, np.newaxis]
        if args.robot == 'freeBody':
            new = utils.interpolate_freeBody(pose1, pose2, nearest_neighbors.dist_freeBody)
        else:
            new = utils.interpolate_arm(pose1, pose2, nearest_neighbors.dist_arm)
        final_path = final_path + new
    dists = []
    if args.robot == 'freeBody': 
        fig, axs = plt.subplots(1, 2, figsize=(15, 8))
        axs = axs.flatten()
        for ax in axs:
            ax.set_aspect('equal')
            ax.set_xlim((-10,10))
            ax.set_ylim((-10,10))
            ax.grid()
        component_1.visualize_scene_diff_axes(axs[0], obstacles)
        component_1.visualize_scene_diff_axes(axs[1], obstacles)
        # utils.draw_graph(axs[0], tree, args.robot)
        utils.draw_path(axs[1], path, args.robot)

        def drawFrame_left(pose):
            return utils.draw_rectangle_and_frame(axs[1], pose)
        
        edges_ordered = tree.get_insertion_order_edges()
        patches_anim = [axs[0].plot([], [], marker='o', c='r', mfc='b')[0] for _ in range(len(edges_ordered))]
        def drawFrame_right(edge_idx):
            edge = edges_ordered[edge_idx]
            edge_patch = patches_anim[edge_idx]
            edge_patch.set_data(edge[:, 0], edge[:, 1])

            return patches_anim      

        anim_left = animation.FuncAnimation(fig, drawFrame_left, frames=final_path, blit=True, interval=300/len(path))
        anim_right = animation.FuncAnimation(fig, drawFrame_right, frames=len(edges_ordered), repeat=False, blit=True, interval=30/len(edges_ordered))
    else:
        fig, axs = plt.subplots(1, 2, figsize=(15, 8))
        axs = axs.flatten()
        
        for ax in axs:
            ax.set_aspect('equal')
            ax.grid()        
        axs[0].set_xlim((-np.pi, np.pi))
        axs[0].set_ylim((-np.pi, np.pi))
        axs[1].set_xlim((-10,10))
        axs[1].set_ylim((-10,10))

        # utils.draw_graph(axs[0], tree, args.robot)
        component_1.visualize_scene_diff_axes(axs[1], obstacles)
        def drawFrame_left(pose):
            return utils.draw_2arm_and_frame(axs[1], pose)
        
        edges_ordered = tree.get_insertion_order_edges()
        patches_anim = [axs[0].plot([], [], marker='o', c='r', mfc='b')[0] for _ in range(len(edges_ordered))]
        def drawFrame_right(edge_idx):
            edge = edges_ordered[edge_idx]
            edge_patch = patches_anim[edge_idx]

            d = np.linalg.norm(edge[0]-edge[1])
            dists.append(d)
            if d <= max_steer_dist:
                edge_patch.set_data(edge[:, 0], edge[:, 1])
            else:
                pass
                # prof said we dont have to plot these types of edges 
            return patches_anim    
        anim_left = animation.FuncAnimation(fig, drawFrame_left, frames=final_path, blit=True, interval=300/len(path))
        anim_right = animation.FuncAnimation(fig, drawFrame_right, frames=len(edges_ordered), repeat=False, blit=True, interval=30/len(edges_ordered))

    if EXPORT:
        # anim_left.save(f"rrt_{args.robot}_{args.map}_path.gif")
        anim_right.save(f"rrt_{args.robot}_{args.map}_tree.gif")

    plt.show() 

def custum_args_arm(start, goal, mapNum):

    parser = argparse.ArgumentParser()
    parser.add_argument('--robot', choices=['arm', 'freeBody'])
    parser.add_argument('--start', nargs='*', type=float)
    parser.add_argument('--goal', nargs='*', type=float)
    parser.add_argument('--goal_rad', type=float)
    parser.add_argument('--map')
    args = parser.parse_args(["--robot", "arm", "--start", str(start[0]), str(start[1]), "--goal", str(goal[0]), str(goal[1]),"--goal_rad", "0.1", "--map", f"map{mapNum}"])
    return args
def custum_args_freeBody(start, goal, mapNum):
    parser = argparse.ArgumentParser()
    parser.add_argument('--robot', choices=['arm', 'freeBody'])
    parser.add_argument('--start', nargs='*', type=float)
    parser.add_argument('--goal', nargs='*', type=float)
    parser.add_argument('--goal_rad', type=float)
    parser.add_argument('--map')
    args = parser.parse_args(["--robot", "freeBody", "--start", str(start[0]), str(start[1]), str(start[2]), "--goal", str(goal[0]), str(goal[1]), str(goal[2]), "--goal_rad", "0.1", "--map", f"map{mapNum}"])
    return args

if __name__ == "__main__":
    if not PART_6_TESTING:
        if EXPORT:
            arm_1_export = custum_args_arm((3.14, 0), (-2, 10.424), 4)
            arm_2_export = custum_args_arm((3.14/8, 0), (-3.14/4*3, 0), 3)
            free_1_export = custum_args_freeBody((9, 9, -4), (9, -9.3, 2), 4)
            free_2_export = custum_args_freeBody((6, 8.5, -10), (-9.2, -2.5, 0), 3)

            main(arm_1_export,  animate=True)
            main(arm_2_export, animate=True)
            main(free_1_export,  animate=True)
            main(free_2_export,  animate=True)
        else:
            a = get_args()
            main(a, animate=True)


    if PART_6_TESTING:
        args_arm = [custum_args_arm((-3.14, 0), (-3.14/4, -3.14/4), 0),
                    custum_args_arm((3.14/4, 0), (-3.14/4*3, 0), 1),
                    custum_args_arm((3.14/4*3, 0), (-3.14/2, 0), 2),
                    custum_args_arm((3.14/8, 0), (-3.14/4*3, 0), 3),
                    custum_args_arm((0, 0), (-2, 10.424), 4)]
        stats_arm = []
        for arg in args_arm:
            data = []
            for _ in range(ITER_TESTING):
                run = main(arg)
                data.append(run)
            data = np.asarray(data)
            avg = np.average(data, axis=0)
            
            a = avg[-1]
            avg[-1] = np.average(data[np.isfinite(data[:, -1])], axis=0)[-1]
            print(a, "vs", avg[-1])
            stats_arm.append(avg)

        args_free = [custum_args_freeBody((9, 9, -2), (-9, -9, 3.14/2), 0),
                 custum_args_freeBody((-9, 9, 5), (9, -9, 3.14/4), 1),
                 custum_args_freeBody((6.5, 1, 3.14/4*3), (-9, 0, 1.3), 2),
                 custum_args_freeBody((6, 8.5, -10), (-9.2, -2.5, 0), 3),
                 custum_args_freeBody((9, 9, -4), (9, -9.3, 2), 4)]
        stats_free = []
        for arg in args_free:
            data = []
            for _ in range(ITER_TESTING):
                run = main(arg)
                data.append(run)
            data = np.asarray(data)
            avg = np.average(data, axis=0)
            
            a = avg[-1]
            avg[-1] = np.average(data[np.isfinite(data[:, -1])], axis=0)[-1]
            print(a, "vs", avg[-1])
            stats_free.append(avg)

        print("Arm Robot Testing")
        for i, (path_found, run_time, path_cost) in enumerate(stats_arm):
            print(f"- Run {i} done {ITER_TESTING} Times")
            print(f"\tPath Found %: {path_found}")
            print(f"\tAVG Run Time (s): {run_time}")
            print(f"\tAVG Path Cost: {path_cost}")
        print("FreeBody Robot Testing")
        for i, (path_found, run_time, path_cost) in enumerate(stats_free):
            print(f"- Run {i} done {ITER_TESTING} Times")
            print(f"\tPath Found %: {path_found}")
            print(f"\tAVG Run Time (s): {run_time}")
            print(f"\tAVG Path Cost: {path_cost}")
