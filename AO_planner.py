...
RUN, K = 500 if PART_6_TESTING else 5000, int(2*np.e)
def get_args():
...
def make_prm_star(obstacles, robot_type, rand_pose, near, collisonOccurs, localPlanner, dist):
    ...
    for _ in range(RUN):
        ...
        l = 1 if len(graph._graph) == 0 else len(graph._graph)
        k_nearest = near(robot_type, int(K*np.log(l)), pose_rand.flatten(), graph.get_vertices())
...
def query_prm_star(graph, obstacles, robot_type, start, goal, near, collisonOccurs, localPlanner, dist):
    # connect start and goal to graph to the nearest node that it can get to w/o collison
    for pose in [start, goal]:
        l = 1 if len(graph._graph) == 0 else len(graph._graph)
        all_ranked = near(robot_type,  int(K*np.log(l)), pose.flatten(), graph.get_vertices())
        connected = False
        ...
        if not connected:
            # if the for loop above runs through w/o a break link
...
def main(args, animate=False):
    ...
    # utils.draw_graph(ax, graph, args.robot)
    # plt.show()
    # delete
    ...
    if args.robot == 'freeBody':
        path, cost = query_prm_star(graph, obstacles,
                        args.robot,
                        start, 
                        goal,
                        nearest_neighbors.get_k_nearest, 
    ...
    # f = open("Temp", 'w')
    # pprint.pprint(graph._graph, stream=f)
    # print(path.shape)
    ...
    for i in range(path.shape[1]-1):
        ...
        # print("Run", i, "->", i+1)
        # print(pose1.flatten(), new[0].flatten())
        # print(pose2.flatten(), new[-1].flatten())
        final_path = final_path + new
    # print("Final Len Of Path", len(final_path))
    # print("Last Finla")
    # print(path[:,0].flatten(), final_path[0].flatten())
    # print(path[:,-1].flatten(), final_path[-1].flatten())
    ...
    if args.robot == 'freeBody': 
        ...
        anim = animation.FuncAnimation(fig, drawFrame, frames=final_path, blit=True, interval=300/len(path))
    # if EXPORT:
    #     anim.save(f"prm*_{args.robot}_{args.map}_path.gif")
    plt.show()
